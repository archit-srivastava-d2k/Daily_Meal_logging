...
import { ChevronDown, Check, Minus, RotateCcw } from 'lucide-react';
...
export function MealCard({
  ...
  const handleRepeatYesterday = () => {
    if (yesterdayMeal) {
      ...
      const items = yesterdayMeal.split(',').map(item => item.trim()).filter(Boolean);
  ...
  const getStatus = () => {
    if (data.skipped) return { label: 'Skipped', color: 'text-muted-foreground' };
    if (data.items.length > 0 && data.feeling) return { label: 'Logged', color: 'text-success' };
    if (data.items.length > 0) return { label: 'In progress', color: 'text-warning' };
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            ...
            <span className={`text-sm ${status.color}`}>
              {status.label === 'Logged' && <Check className="inline w-3 h-3 mr-1" />}
              {status.label === 'Skipped' && <Minus className="inline w-3 h-3 mr-1" />}
              {status.label}
            </span>
          </div>
        </div>
                    ...
                    <div className="space-y-2">
                      {data.items.map((item, index) => (
                        <PortionSelector
                          ...
                          onSelect={(portion) => handlePortionSelect(item, portion)}
                        />
                      ))}
                    </div>
                  )}
